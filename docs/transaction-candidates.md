...
- Standby deployment updates the standby day, clears any previous covered route, updates the newly covered route, and synchronizes generated issues in one transaction. A partial unique index also guarantees that only one deployed standby can cover a `(date, coveringRoute)` tuple, including shared branch pools.
